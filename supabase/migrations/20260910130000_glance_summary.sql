-- Adds a one-sentence, always-populated summary of WHO this person is
-- (their likely role, and whether they're at a specific campus or the
-- district office) so a reviewer gets that at a glance on the card itself,
-- instead of only inside the "Show match reasoning" prose, which research
-- already derives (institutionLevel/titleFinding) but never surfaced
-- outside a wall of text a reviewer has to expand and read in full.
alter table public.contacts add column glance_summary text;

drop function if exists public.finalize_contact_match(
  uuid, text, text, text, text, text, text, text, text, text, text, boolean, text, jsonb, text, text, boolean, boolean
);
