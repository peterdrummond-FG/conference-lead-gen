-- Threads the new qr_channel column (previous migration) through
-- insert_contact_with_duplicate_check's payload -> column mapping, same as
-- every other contacts-create-only field. search_path is re-pinned inline
-- here rather than relying on the standalone ALTER from
-- 20260908133000_pin_search_path_new_fns.sql surviving a CREATE OR REPLACE.
...
begin
  ...
  insert into public.contacts (
    event_id, source, qr_channel, first_name, last_name, email, phone, title, state,
