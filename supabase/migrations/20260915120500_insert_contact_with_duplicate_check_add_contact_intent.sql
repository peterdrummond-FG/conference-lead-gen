-- Threads contact_intent through insert_contact_with_duplicate_check's
-- payload -> column mapping, same as qr_channel/rep_id before it. Needed so
-- contacts-create can set contact_intent='hot' at insert time for a
-- session-QR submission (see 20260910140000_add_contact_intent.sql) --
-- someone who scans the breakout-session QR mid-talk has self-selected as
-- engaged, which is a real signal worth starting hot on rather than null.
-- contact_intent_is_manual is left at its default (false), so
-- local-agent's intentLoop still takes over the moment real interaction
-- notes show up for that contact -- this is a starting value, not a lock.
